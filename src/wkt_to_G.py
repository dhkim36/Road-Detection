...
# import cv2
from other_tools import make_logger, rdp
...
def get_node_geo_coords(G, im_file, fix_utm_zone=True, n_threads=12,
    ...
    if n_threads > 1:
        ...
        for p in params:
            coords_dict_list.update(pixelToGeoCoord(p))
        # coords_dict_list = pixelToGeoCoord(params[0])

    # combine the disparate dicts
    coords_dict = {}
    print('===========================')
    print('coords_dict_list :: ', coords_dict_list)
    print('===========================')
    coords_dict.update(coords_dict_list)
...
def get_edge_geo_coords(G, im_file, remove_pix_geom=True, fix_utm_zone=True,
    ...
    if n_threads > 1:
        ...
        for p in params:
            coords_dict_list.update(convert_pix_lstring_to_geo(p))
...
def wkt_to_G(params):
    ...
    if simplify_graph:
        ...
        key_tmp = 'geometry_utm_wkt'  # 'geometry_wkt'   # 'geometry_utm_wkt'
        for i, (u, v, attr_dict) in enumerate(Gout.edges(data=True)):
            if verbose and ((i % 10000) == 0):
...
def main():
    ...
    # outut files
    res_root_dir = '/home/dhkim/work/RoadDetector/albu-solution/results'
    path_images = '/home/dhkim/work/RoadDetector/albu-solution/data/test/images'
    csv_file = os.path.join(res_root_dir, 'skeletonize_results.csv')
