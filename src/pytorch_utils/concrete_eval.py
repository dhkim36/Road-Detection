...
class CropEvaluator(Evaluator):
    ...
    def save(self, name, prediction, prefix=""):
        result_array = self.postprocess(prediction)
        cv2.imwrite(os.path.join(self.save_dir, prefix + name), result_array)

    def postprocess(self, prediction):
        # making image
        result_array = (prediction * 255).astype(np.uint8)

        ## predict threshold
        result_array = np.where(result_array < 70, 0, result_array)
        result_array = np.where(result_array >= 70, 255, result_array)

        ## erosion image
        k = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        result_array = cv2.dilate(result_array, k)
        result_array = cv2.dilate(result_array, k)
        result_array = cv2.dilate(result_array, k)
        result_array = cv2.erode(result_array, k)
        result_array = cv2.erode(result_array, k)
