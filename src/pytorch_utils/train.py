...
def train(ds, fold, train_idx, val_idx, config, val_ds=None, num_workers=0, transforms=None, val_transforms=None):
    ...
    TrD = TrainDataset(ds, train_idx, config, transforms=transforms)
    train_loader = PytorchDataLoader(TrD,
                                     batch_size=config.batch_size,
                                     shuffle=True,
                                     drop_last=True,
                                     num_workers=num_workers,
                                     pin_memory=True)
    VaD = ValDataset(val_ds if val_ds is not None else ds, val_idx, config, transforms=val_transforms)
    val_loader = PytorchDataLoader(VaD,
