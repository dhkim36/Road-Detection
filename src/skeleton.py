...
def preprocess(img, thresh):
    ...
    #img = remove_small_objects(img, 800)
    print(' - remove_small_holes ...')
    img = remove_small_holes(img, 1200)
...
def make_skeleton(root, fn, debug, threshes, fix_borders):
    ...
    print('img read ...')
    img = io.imread(os.path.join(root, fn))
...
def build_graph(root, fn, debug=False, threshes={'2': .3, '3': .3, '4': .3, '5': .2}, add_small=True, fix_borders=False):
    ...
    ### ske imwrite !!
    print('ske imwrite ...')
    r_root = r'/home/dhkim/work/RoadDetector/albu-solution/results'
    save_dir = os.path.join(r_root, 'Contec', 'skeleton_vis')
    save_path = os.path.join(save_dir, fn)
    cv2.imwrite(save_path, ske.astype(np.uint8)*255)

    print('sknw build_sknw ...')
    G = sknw.build_sknw(ske, multi=True)
    print('G :: ', G)
    print('remove_small_terminal...')
...
if __name__ == "__main__":
    ...
    prefix = ''
    results_root = r'/home/dhkim/work/RoadDetector/albu-solution/results'
    skeleton_results_dir = os.path.join(results_root, 'Contec', 'skeleton_vis')
    csv_save_dir = os.path.join(results_root, 'Contec', 'graph_csv')
    os.makedirs(skeleton_results_dir, exist_ok=True)
    os.makedirs(csv_save_dir, exist_ok=True)
    # results_root = r'd:\tmp\roads\albu\results\results'
    # root = os.path.join(results_root, r'results\2m_4fold_512_30e_d0.2_g0.2')
    root = os.path.join(results_root, 'Contec', 'Contec_test')
    ...
    for k, v in tqdm(data):
        ...
        output_file = os.path.join(csv_save_dir, k+'.csv')
